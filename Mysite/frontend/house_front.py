...
FullBath = st.number_input("Количество ванных", min_value=0, max_value=10, step=1)
OverallQual = st.slider("Качество", min_value=1, max_value=10)
Neighborhood = st.selectbox('Район', ['Blueste', 'BrDale', 'CollgCr', 'Gilbert', 'Mitchel', 'NWAmes', 'OldTown', 'SawyerW',
...
if st.button('Проверка'):
    try:
        ...
        if answer.status_code == 200:
            result = answer.json()
            st.success(f'Результат: {result.get("Predict_Price")}')
        else:
